...
def split_dataset(skes_joints, label, performer, setup, evaluation, save_path):
    ...
    train_indices = train_indices[train_indices < len(skes_joints)]
    test_indices = test_indices[test_indices < len(skes_joints)]
...
def get_indices(performer, setup, evaluation='CSub'):
    ...
    if evaluation == 'CSub':  # Cross Subject (Subject IDs)
        ...
        for idx in test_ids:
            ...
            temp = temp[temp < dataset_size]  # Ensure valid indices
            test_indices = np.hstack((test_indices, temp))

        for train_id in train_ids:
            temp = np.where(performer == train_id)[0]
            temp = temp[temp < dataset_size]  # Ensure valid indices
            train_indices = np.hstack((train_indices, temp))
    else:  # Cross Setup (Setup IDs)
        train_ids = [i for i in range(1, 33) if i % 2 == 0]
        test_ids = [i for i in range(1, 33) if i % 2 == 1]

        for test_id in test_ids:
            temp = np.where(setup == test_id)[0]
            temp = temp[temp < dataset_size]  # Ensure valid indices
            test_indices = np.hstack((test_indices, temp))

        for train_id in train_ids:
            temp = np.where(setup == train_id)[0]
            temp = temp[temp < dataset_size]  # Ensure valid indices
            train_indices = np.hstack((train_indices, temp))

    # Ensure all indices are within bounds
    train_indices = train_indices[train_indices < dataset_size]
    test_indices = test_indices[test_indices < dataset_size]
